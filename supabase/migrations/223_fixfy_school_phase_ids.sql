-- Migration 223: Extend Fixfy School phase_id checks (fixfy-products + ops-playbook).
...
ALTER TABLE public.fixfy_school_quiz_attempts
  ...
  CHECK (phase_id IN ('fixfy-products', 'zendesk', 'fixfy-os', 'trade-portal', 'ops-playbook'));

COMMENT ON COLUMN public.profiles.fixfy_school_certified_phases IS
  'Phase ids with quiz passed 5/5: fixfy-products, zendesk, fixfy-os, trade-portal, ops-playbook.';
...
BEGIN
  ...
  IF p_phase_id NOT IN ('fixfy-products', 'zendesk', 'fixfy-os', 'trade-portal', 'ops-playbook') THEN
  ...
  INSERT INTO public.fixfy_school_progress (
    ...
    p_last_lesson_id,
    COALESCE(p_unlocked_at, '{"fixfy-products": null}'::jsonb),
