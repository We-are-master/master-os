...
export async function POST() {
  ...
  for (const [, groupJobs] of groups) {
    ...
    for (const j of groupJobs) {
      const { error: uErr } = await supabase.from("jobs").update({ self_bill_id: sbId }).eq("id", j.id);
      if (!uErr) jobsLinked += 1;
    }

    const { data: jl } = await supabase.from("jobs").select("partner_cost, materials_cost").eq("self_bill_id", sbId);
    const list = jl ?? [];
    let jobValue = 0;
    let materials = 0;
    for (const r of list) {
