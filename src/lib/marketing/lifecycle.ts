 ...
 *   4b. e-mail de empresa           é do plano B2B, não deste funil.
...
export function contasNossas(): Set<string> {
  const ids = [
...
export type ResultadoDaVarredura = {
  ...
  bloqueados: number;
  plataforma: number;
  /** E-mail de domínio de empresa: é do plano B2B, não do funil de limpeza. */
  empresa: number;
  semEmail: number;
...
export async function varrerFunil(opcoes: { aplicar: boolean; limite?: number }): Promise<ResultadoDaVarredura> {
  ...
  const { data, error } = await sb
    ...
    // O teto do dia deixa entrar uns poucos por vez: quem comprou vem primeiro,
    // depois o lead mais novo, que ainda lembra de ter pedido preço.
  ...
  for (const c of brutos) {
    ...
    /**
     * Trava 4b: só caixa pessoal. O funil vende limpeza para quem mora no
     * imóvel; a caixa `@imobiliaria.co.uk` é de quem gere cem imóveis e tem
     * conversa própria no plano B2B. Decisão do dono em 23/09/2026: "todos os
     * leads B2C, sem o @company".
     */
    if (segmentoDoEmail(email) === "b2b") { res.empresa++; continue; }
