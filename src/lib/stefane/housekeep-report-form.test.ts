...
test("sem horário de obra não se envia: a plataforma exige os dois", () => {
  /**
   * JOB-9449, 20/08/2026. O parceiro nunca ligou o timer, o envio subiu as 60
   * fotos, levou 441 segundos e voltou recusado com "fields not accepted:
   * Start time, Finish time". Sete minutos para descobrir uma coisa que dava
   * para saber antes de abrir o navegador.
   */
  const limpeza = payloadLimpeza({
    start: {},
    final: { job_complete: true },
    inicio: null,
    fim: null,
  });
  assert.equal(limpeza.ok, false);
  assert.match(limpeza.ok ? "" : limpeza.motivo, /start and finish times/);

  const trade = payloadDoReport({
    final: { description: "Fixed the tap." },
    inicio: "2026-08-20T08:00:00Z",
    fim: null,
  });
  assert.equal(trade.ok, false);
  assert.match(trade.ok ? "" : trade.motivo, /the finish time/);
});
